...
export class StartupService implements OnApplicationBootstrap {
  ...
  private async createRootUser(): Promise<void> {
    ...
    const adminExists: boolean = await this._usersService.hasAnyWithModel({
      role: Role.Administrator,
    });

    if (adminExists) {
      this._logger.log(
        "One or more Administrator users exist - skipping root user creation."
      );
      return;
    }
    ...
    if (!rootUsername || !rootPassword) {
      throw new Error(
        "Root user username and/or password has not been defined!"
      );
    }
    ...
    this._logger.log(
      "Root user created & inserted into database successfully."
    );
  }
