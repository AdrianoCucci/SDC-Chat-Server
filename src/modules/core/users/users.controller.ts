...
export class UsersController {
  ...
  public async getAllUsers(
    @Query() model?: Paged<Includable<UserDto>>,
    @Includes() includes?: string[]
  ): Promise<PagedList<UserDto>> {
    const { skip, take, include, ...rest } = model;

    const result: PagedList<UserDto> = await catchEntityColumnNotFound(
      async () => {
  ...
  public async postUser(
    ...
    const secret: UserSecret = await generateUserSecret(
      entity.id,
      request.password
    );
  ...
  private async validatePostModel(
    ...
    if (
      ...
      requestUser.role !== Role.Administrator
    ) {
      errors.push("organizationId is required");
    } else if (
      request.organizationId != null &&
      !(await this._orgService.hasAnyWithId(request.organizationId))
    ) {
    ...
    if (requestUser.role !== Role.Administrator) {
      ...
      request.role =
        request.role === Role.Administrator ? Role.User : request.role;
    }
  }

  private async validatePutModel(
    requestUser: UserDto,
    entity: User,
    request: PartialUserDto
  ): Promise<void> {
    const errors: string[] = [];

    if (request.username) {
      const userByUsername: User = await this._usersService.getByUsername(
        request.username
      );
    ...
    if (
      requestUser.id !== entity.id &&
      !userIsInRole(requestUser, [Role.Administrator, Role.OrganizationAdmin])
    ) {
